...
export function PixelField({ label, hint, children }) {
  return (
    <div className="border-t-2 border-deep/10 pt-4 first:border-t-0 first:pt-0 dark:border-cream/10">
      {label && (
      ...
      {children}
      {hint && <p className="text-xs text-deep/50 dark:text-cream/50 mt-1.5">{hint}</p>}
    </div>
  )
}
